...
class STCameraView: UIView {
    ...
    /// 相机(默认前置摄像头)
    private lazy var camera : GPUImageVideoCamera? = GPUImageVideoCamera(sessionPreset: AVCaptureSession.Preset.high.rawValue, cameraPosition: .front)
...
extension STCameraView {
    ...
    func startBeautiful() {
        ...
        let groupFilters = getGroupFilters()
        camera?.addTarget(groupFilters)
        groupFilters.addTarget(preview)
    }
